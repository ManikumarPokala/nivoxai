...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Campaigns proxy failed.";
    return errorResponse(502, requestId, "Campaigns proxy failed.", { error: message });
  }
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Campaign create proxy failed.";
    return errorResponse(502, requestId, "Campaign create proxy failed.", { error: message });
  }
}
