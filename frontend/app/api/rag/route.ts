...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const response = await fetch(`${BACKEND_API_BASE_URL}/rag/influencers`, {
      method: "POST",
      headers,
    ...
    const message = error instanceof Error ? error.message : "RAG proxy failed.";
    return errorResponse(502, requestId, "RAG proxy failed.", { error: message });
  }
}
