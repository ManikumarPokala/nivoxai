...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Agent status proxy failed.";
    return errorResponse(502, requestId, "Agent status proxy failed.", { error: message });
  }
}
